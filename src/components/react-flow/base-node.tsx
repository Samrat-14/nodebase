...
export function BaseNode({ className, status, children, ...props }: BaseNodeProps) {
  return (
    ...
    >
      {children}
      {status === 'error' && (
        <XCircleIcon className="absolute right-0.5 bottom-0.5 size-2 text-red-700/75 stroke-3" />
      )}
      {status === 'success' && (
        <CheckCircle2Icon className="absolute right-0.5 bottom-0.5 size-2 text-emerald-700/75 stroke-3" />
      )}
      {status === 'loading' && (
        <Loader2Icon className="absolute -right-0.5 -bottom-0.5 size-2 text-blue-700/75 stroke-3 animate-spin" />
      )}
    </div>
  );
}
